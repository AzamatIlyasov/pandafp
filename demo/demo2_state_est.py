import pandapower as pp

#create empty net
net = pp.create_empty_network() 

#create buses
b1 = pp.create_bus(net, vn_kv=20., name="bus 1", vn_kv=1., index=1)
b2 = pp.create_bus(net, vn_kv=0.4, name="bus 2", vn_kv=1., index=2)
b3 = pp.create_bus(net, vn_kv=0.4, name="bus 3", vn_kv=1., index=3)

#create bus elements
pp.create_ext_grid(net, bus=b1, vm_pu=1.02, name="Grid Connection")
pp.create_load(net, bus=b3, p_mw=0.1, q_mvar=0.05, name="Load")

#create branch elements
tid = pp.create_transformer(net, hv_bus=b1, lv_bus=b2, std_type="0.4 MVA 20/0.4 kV", name="Trafo")
line = pp.create_line(net, from_bus=b2, to_bus=b3, length_km=0.1, name="Line",std_type="NAYY 4x50 SE") 

print(net)



pp.runpp(net)
print(net.res_bus)
